...
Func dim1e2()
	...
	For $ii = 1 To $ListaArquivos[0] Step 1
		...
		;$dim1 = StringMid($dim1, 1, $posicao)
		;MsgBox(4096, Default, "dimensões:" & @CRLF & $dim1 )
		$xx[$ii] = $dim1 ; / 72 * 2.54  show in mm
		;$yy[$ii] = Int($dim2) ; / 72 * 2.54 show in mm

		$bData = StringToBinary("/Linearized")
		$Offset = _HexSearch($TestFile_Path, $bData)
		$data = _HexRead($TestFile_Path, $Offset, '80')
		$data = StringTrimLeft($data, 2)
		$data = _HexToString($data)
		;MsgBox(4096, Default, "texto paginas depois:" & @CRLF & $data)
		$result1 = StringInStr($data, "/N")+2
		;MsgBox(0, "Search result:", $result1)
		$result2 = StringInStr($data, "/T")
		;MsgBox(0, "Search result:", $result2)
		$compr1 = stringlen($data)+1
		$data = StringTrimRight($data, $compr1-$result2)
		;MsgBox(0, "Search result:", $data)
		$pags[$ii] = StringTrimLeft($data, $result1)
		;MsgBox(0, "Search result:",$pags[$ii])
		$tttt = $tttt & $ListaArquivos[$ii] & Chr(09) & $xx[$ii]  & Chr(13) ;& Chr(09) & $yy[$ii] & Chr(09) & $pags[$ii] & Chr(13)
	Next
	_ArrayDisplay( $xx, "1" )
	;_ArrayDisplay( $yy, "2" )
